// api/thu-cron.js
// 국민건강보험공단 '필기 공략 12부작' 카드뉴스 자동 발행 — 탑커리어(top_career_ · 2번 계정)
//   · 매주 목요일 19:00 KST (vercel.json: 0 10 * * 4)
//   · 캐러셀 6장(표지→내용4→프리미엄) + 캡션/해시태그 20개 (thu-data.js)
//   · 진도는 Blob(thu-progress.json)에 기록, 12편 소진 후 1편부터 재순환
//
// 필요한 환경변수: IG_USER_ID_2, IG_ACCESS_TOKEN_2, CRON_SECRET, PUBLISH_SECRET, BLOB_READ_WRITE_TOKEN
// 카드 이미지: 레포 루트의 /thu-cards/<파일명> (정적 호스팅)
//
// 수동 테스트: /api/thu-cron?secret=<PUBLISH_SECRET>&dryrun=1   (미리보기)
//             /api/thu-cron?secret=<PUBLISH_SECRET>             (실제 발행)
//             &day=3 을 붙이면 특정 회차 강제 지정(1~12).

const DAYS = require('./thu-data.js');
...
const CARDS_PATH = '/thu-cards';
const PROGRESS_KEY = 'thu-progress.json';
...
/* 캐러셀 발행 (2번 계정) — 6장이라 처리 대기·재시도 포함 */
...
module.exports = async (req, res) => {
  ...
  try{
    if(!DAYS.length) return out(500, { ok:false, error:'thu 데이터가 없습니다.' });

    const prog = await readProgress();
    let idx;
    const forced = req.query && parseInt(req.query.day, 10);
    if(forced && forced >= 1 && forced <= DAYS.length){
      idx = forced - 1;
    } else {
      idx = ((prog.dayIndex || 0) % DAYS.length + DAYS.length) % DAYS.length;
    }
    const item = DAYS[idx];
    ...
    return out(200, { ok:true, published:true, day: item.day, title: item.title,
      mediaId, nextIndex: prog.dayIndex, total: DAYS.length });
  }catch(err){
